...
public class ChatDAO {
    ...
    public ArrayList<Chat> getChatListByID(String userID, String roomID, String chatID) {
        ...
        ResultSet rs = null;
        String SQL = "SELECT * FROM CHAT WHERE roomID = ? AND chatID > ? ORDER BY chatTime";
        ...
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        String SQL ="SELECT * FROM CHAT WHERE ((userID = ? AND roomID = ?) OR (userID = ? AND roomID = ?)) AND chatID > (SELECT MAX(chatID) - ? FROM CHAT) ORDER BY chatTime";
		try {
			pstmt=conn.prepareStatement(SQL);
			// 다음 parameter 들로 sql 쿼리문 안에 있는 ?문자를 대체한다.
